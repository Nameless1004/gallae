...
- "할 일이 너무 많다" → Eisenhower Matrix / OKR.
- "위험·실패 걱정" → Pre-mortem / Risk Matrix.
- "사용자/제품" → Jobs-to-be-Done.
...
- "원인이 여러 개로 보인다" → Root Cause Tree 또는 Fishbone (Ishikawa).
- "상황을 한눈에 파악하고 싶다", "강점·약점을 정리하고 싶다" → SWOT.
- "빠르게 결정하고 행동해야 한다", "관찰→행동 사이클이 막혔다" → OODA Loop.
- 단순 회고면 STAR 회고.
...
- "Fishbone (Ishikawa)": 가지는 문제 유형에 맞는 원인 카테고리(사람/방법/환경/도구/측정/재료)를 고릅니다. leaf는 그 카테고리 안에서 실제로 확인 가능한 원인 후보여야 합니다. 원인 추측이 아니라 관찰·측정 가능한 항목으로 쓰세요.
- "Jobs-to-be-Done": 가지는 기능적 Job/감정적 Job/사회적 Job/맥락을 기본으로 합니다. leaf는 사용자가 실제로 처한 상황과 원하는 결과를 구체적으로 담아야 합니다. "더 좋게 만들기"처럼 막연한 표현은 금지합니다.
- "First Principles": 가지는 자명한 사실/숨은 가정/재구성된 문제를 기본으로 합니다. leaf는 현재 당연하게 받아들이는 가정을 하나씩 의심하는 질문이어야 합니다. 기존 방식을 개선하는 수준으로 끝내지 말고 전제 자체를 뒤집으세요.
- "Eisenhower Matrix": 가지는 긴급중요/중요(긴급X)/긴급(중요X)/제거를 기본으로 합니다. leaf는 사용자가 실제로 안고 있는 구체 태스크나 결정이어야 합니다. 카테고리 설명이 아니라 배치할 항목을 쓰세요.
- "OODA Loop": 가지는 관찰/판단/결정/행동을 기본으로 합니다. leaf는 각 단계에서 지금 당장 실행 가능한 확인·선택·실행 항목이어야 합니다. 분석에만 머물지 말고 행동 가지에서 구체 동사를 반드시 포함하세요.
- "STAR 회고": 가지는 상황/과제/행동/결과·학습을 기본으로 합니다. leaf는 그 사건에서 실제로 있었던 사실이나 내린 판단이어야 합니다. 미래 계획이 아니라 과거 사건 재구성에 집중하세요.
- "SWOT": 가지는 강점/약점/기회/위협을 기본으로 합니다. leaf는 사용자 문제 맥락에서 실제로 확인 가능한 항목이어야 합니다. "더 잘할 수 있다" 같은 희망 문장이 아니라 지금 관찰 가능한 사실로 쓰세요.
- "ICE/RICE": 가지는 ICE(영향/확신/노력) 또는 RICE(도달범위/영향/확신/노력)로 분류합니다. leaf는 각 기준에 따라 평가할 수 있는 구체 항목이나 측정 방법이어야 합니다. 점수를 임의로 단정하지 말고 어떻게 측정할지를 쓰세요.
- "Stakeholder Map": 가지는 이해관계자/욕구/충돌/조정 포인트를 기본으로 합니다. leaf는 실제 관련자나 그들의 구체 요구·갈등 지점이어야 합니다. 추상적인 "소통 강화" 같은 표현은 금지합니다.
- "Risk Matrix": 가지는 고위험/중위험/저위험/완화책을 기본으로 합니다. leaf는 사용자 상황에서 실제로 발생 가능한 리스크 항목이어야 합니다. 일반적인 위험 목록이 아니라 이 문제에 닿는 구체 사례로 쓰세요.
- "Assumption Mapping": 가지는 핵심 가정/불확실성/검증 방법/학습 기준을 기본으로 합니다. leaf는 지금 사실로 여기고 있지만 틀릴 수 있는 가정 하나여야 합니다. "확인이 필요하다" 수준으로 끝내지 말고 어떻게 반증할지를 쓰세요.
- "이슈 트리 (MECE)": 가지는 문제를 겹치지 않고 빠짐없이 나누는 하위 질문들이어야 합니다. leaf는 그 질문에 답하기 위해 확인할 데이터·조건·판단 기준이어야 합니다. 같은 가지 안에서 중복되는 항목이 없도록 하세요.
- "6 Hats (de Bono)": 가지는 사실/감정/리스크/낙관/창의/메타 중 문제에 맞는 4~5개를 선별합니다. leaf는 그 관점에서 실제로 보이는 것, 느끼는 것, 생각하는 것이어야 합니다. 관점끼리 섞이지 않도록 각 가지의 렌즈를 유지하세요.
- "OKR": 가지는 목표/핵심결과/리스크/지표를 기본으로 합니다. leaf는 실제로 달성 여부를 측정할 수 있는 결과나 지표여야 합니다. "더 잘한다" 같은 측정 불가 표현은 금지합니다.
- "Impact-Effort Matrix": 가지는 고효과저노력/고효과고노력/저효과저노력/제외를 기본으로 합니다. leaf는 사용자의 실제 태스크나 선택지를 배치한 것이어야 합니다. 카테고리 설명이 아니라 해당 칸에 들어갈 구체 항목을 쓰세요.
...
leverage 배분 가이드:
- 각 branch 당 high는 1~2개로 제한합니다. 모두 high면 우선순위가 없는 것과 같습니다.
- medium은 2~3개, low는 0~1개가 자연스러운 비율입니다.
- 전체 leaves 중 high 비율이 50%를 넘지 않도록 합니다.
- 같은 branch 안의 모든 leaf를 같은 leverage로 설정하지 마세요.
